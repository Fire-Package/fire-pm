...
export const LogViewer: React.FC<LogViewerProps> = ({
  ...
  return (
    <div className={`telemetry-panel overflow-hidden flex flex-col ${className}`}>
      {/* Top Toolbar */}
      <div className="flex flex-wrap items-center justify-between gap-3 px-3.5 py-2 bg-white/[0.015] border-b border-white/[0.05]">
        <div className="flex items-center gap-2">
          <div className="p-1 rounded-md bg-white/[0.04] text-[#ff5500]">
          ...
          <div className="flex items-center gap-2 text-xs font-mono">
            ...
            <span className="text-[10px] text-slate-500 font-medium px-1.5 py-0.2 rounded bg-white/[0.03] border border-white/[0.05]">
              {logs.length} lines
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="bg-[#06080d] border border-white/[0.06] rounded-md text-xs px-2.5 pl-8 py-1 text-slate-200 placeholder-slate-500 focus:outline-none focus:border-[#ff5500]/70 transition-colors font-mono"
            />
          </div>
          ...
          <Button size="sm" variant="secondary" onClick={handleCopy} title="Copy logs">
          ...
          {onClear && (
            <Button size="sm" variant="ghost" onClick={onClear} title="Clear window">
      ...
      <div
        ...
        className="flex-1 p-3.5 font-mono text-xs text-slate-200 overflow-y-auto overflow-x-auto min-h-[300px] max-h-[600px] leading-relaxed space-y-0.5 selection:bg-[#ff5500]/30 select-text bg-[#05070c]"
      >
        {filteredLogs.length === 0 ? (
          <div className="text-slate-500 italic py-12 text-center font-mono text-xs">
            {filter ? `No log entries matching "${filter}"` : "Waiting for incoming Journalctl log stream..."}
          </div>
        ) : (
